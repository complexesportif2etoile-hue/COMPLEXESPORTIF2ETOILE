/*
  # Add admin check inside restore_backup function

  ## Summary
  Updates restore_backup to check that the calling user is an admin.
  This allows the function to be called directly from the frontend
  (bypassing the edge function) without compromising security.

  The admin check uses auth.uid() to look up the calling user's role.
  If the caller is not an admin, the function raises an exception immediately.
*/

DROP FUNCTION IF EXISTS public.restore_backup(jsonb, jsonb, jsonb, jsonb, jsonb, jsonb, jsonb, jsonb, jsonb);
