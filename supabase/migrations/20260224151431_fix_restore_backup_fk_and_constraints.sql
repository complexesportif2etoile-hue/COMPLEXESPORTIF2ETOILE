/*
  # Fix restore_backup to handle FK violations and constraint issues

  ## Problem
  The restore_backup function was failing silently because:
  1. reservations.created_by FK references profiles(id) — restored reservations
     may have created_by values that don't exist in the current profiles table
  2. payment_method is NOT NULL — old backups may not have this column

  ## Fix
  Before inserting reservations, nullify created_by values that don't exist
  in the current profiles table, and set defaults for missing NOT NULL columns.
  The function now returns detailed error messages instead of silently swallowing them.
*/
    ...
    BEGIN
      SELECT jsonb_agg(
        ...
        jsonb_build_object(
          ...
          END,
          'payment_method',
          COALESCE(r->>'payment_method', 'ON_SITE'),
          'payment_status',
      ...
      SELECT * FROM jsonb_populate_recordset(null::reservations, v_sanitized_reservations);
    EXCEPTION WHEN OTHERS THEN
